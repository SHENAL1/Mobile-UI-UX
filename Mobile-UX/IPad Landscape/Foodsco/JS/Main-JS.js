  ...
  $.getJSON("products.json", function (data) {
    ...
    $.each(data, function (key, value) {
      product_data +=
        ...
        value.prod_id +
        '">' +
        '<a href="SingleProduct.html?prod_id=' +
        value.prod_id +
        '" rel="external">' +
        '<img src="../Images/Products/' +
        value.prod_img +
        '" alt="prod_img" class="prod_img" id="prod_img">' +
        "</a>" +
        '<hr class="hr">' +
        ...
        "</div>" +
        "</div>" +
        '<div class="prod_category" id="prod_category">' +
        value.prod_category +
        "</div>" +
        '<div class="prod_price" id="prod_price">LKR ' +
        value.prod_price +
        "</div>" +
        "</div>";
    });
...
$("#search_box1").keyup(function () {
  $.getJSON("products.json", function (data) {
    ...
    var searchField = $("#search_box1").val();
...
$(".cat_item").on("click", function () {
  ...
  if (searchField != "All") {
    $.getJSON("products.json", function (data) {
      ...
      // document.getElementsByClassName("cat_item").style.color = "#5DC466";
      // document.getElementsByClassName("cat_item").style.background = "white";
...
function feedbackSubmit(){
  ...
  var feedback_msg =document.getElementById("feedback").value;

  feedbackMessages.push({"Name": "Thomas", "Feedback":feedback_msg});

  var popup ='<div class="contact-feedback-response">'+
                  '<div class="popup-title"><p>Thank You</p></div>'+
                  '<div class="popup-description"><p>Thank You for your<br> Feedback</p></div>'+
                  '<div class="popup-btn">'+
  ...
  localStorage.setItem('feedbackMessages',JSON.stringify(feedbackMessages));
    ...
    document.getElementById('submit-popup-overlay').style.display ='none';
