...
$(document).on('pageinit','#product',function() {
    ...
    console.log('adsad on page init')
    ...
    setTimeout(function () {
        ...
        // console.log("Current User in Logged In: ",currentUser);
        ...
        $("#commentsList").click(function (event) {
            ...
            if (comment_type === 'reply') {
                var reply_form_elem =
                    `
		    		<div class="ui-content" id="${abc}">
<!--						<span class="foodsco_subtitle_" style="color: black ; font-weight: 700">Reply:</span>-->
						<textarea rows="5" id="content-${abc}" class="comment-box" placeholder="Body of review(1500)">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries</textarea>

						<span class="ui-grid-solo" >
							<span class="ui-block-a">
                                <span style="display: flex;float:right" >
                                    <button id="addreply-${abc}" class="add-btn" style="width:65px; color: white;background-color: #3F007A;border: 1px solid #3F007A;font-size: 12px;font-weight: 400;margin: 5px;padding: 8px 7px 8px 7px;">Reply</button>
                                    <button id="cancel" class="cancel-btn" onclick="close_reply_form(${abc})" style=" width:65px; padding: 8px 8px 8px 8px;color: #3F007A;background-color: white;border: 1px solid #3F007A;font-weight: 400;margin-right: 10px ;font-size: 12px;">Cancel</button>
                                </span>
                            </span>
						</span>
...
function store_comments() {
    ...
    for ( let i =0 ; i < products.length ; i ++) {
        console.log('--',products[i]['id'],product_id)
...
function show_comment(comment){
    ...
    if(comment.parentId == null) {
        ...
        var display_elem =
            `
			<div class="ui-content" style="border: 1px solid #C4C4C4; background-color:white; border-radius: 5px ;">
				...
				<div class="ui-grid-a">
                    ...
                    <span class="foodsco_content" style="color: grey"> Reviewed a product - ${comment.lastUpdated}</span>
                </div>
            </div>
            ...
            <span span class="foodsco_content">
                 <a style="color: #3F007A;text-decoration: none" href="#" role="button" id="reply-${comment.id}">Reply</a>
            </span>
        ...
        var display_elem =
            ...
            <div class="ui-content" style="padding:0">
                 <li id="comment-${comment.id}">
                 <div class="ui-content" style="border-left: 4px solid indigo; background-color: #DCDCDC">
